...
def multimesh():
    ...
    mesh_1 = RectangleMesh(
        Point(numpy.pi / 10, numpy.pi / 9), Point(numpy.pi / 8, numpy.pi / 7), 5, 8
    )
    multimesh = MultiMesh()
...
def test_assemble_exterior_facet(v, multimesh):
    ...
    v.vector()[:] = 1
    assert numpy.isclose(assemble_multimesh(v * ds), circumference)


@skip_in_parallel
def test_assemble_interface(v, multimesh):
    mesh_1 = multimesh.part(1)
    xmin = mesh_1.coordinates()[:, 0].min()
    xmax = mesh_1.coordinates()[:, 0].max()
    ymin = mesh_1.coordinates()[:, 1].min()
    ymax = mesh_1.coordinates()[:, 1].max()
    interface_length = 2 * (xmax - xmin) + 2 * (ymax - ymin)
    v.vector()[:] = 1
    assert numpy.isclose(assemble_multimesh(v * dI), interface_length)
...
def test_errornorm_L2(f_2, v_high):
    ...
    assert numpy.isclose(
        errornorm(const, v_high, norm_type="L2", degree_rise=3), numpy.sqrt(22) / 6
    )
...
def test_errornorm_H1(f, f_2, v_high):
    v_high.interpolate(f_2)
    assert numpy.isclose(
        errornorm(f, v_high, norm_type="H1", degree_rise=3),
        numpy.sqrt(37.0 / 36 + 5 * numpy.pi**2 / 4),
    )
...
def test_vector_space(v_vec, e_x, f_vec2, V_vector, f_vec):
    ...
    print(
        (
            assemble_multimesh(inner(v_vec, f_vec2) * dX),
